...
conversion_rate = st.slider(
    'Conversion Rate', min_value=0.0, max_value=1.0, value=0.30, step=0.01,
    help='Proportion of leads expected to convert to deals (0 to 1).'
)
...
if st.button('Start Estimation'):
    if len(st.session_state['persons']) == 0:
        ...
        with st.spinner('Estimating...'):
            estimates_raw = []  # per lead (before count and conversion)
            estimates_effective = []  # after count and conversion
            ...
            df['Estimate'] = estimates_effective  # already includes count and conversion rate

            st.markdown('### Visualizations')
            # Group by Technology Interest
            tech_group = df.groupby('technology')['Estimate'].sum().reset_index()
            fig_tech = px.bar(
                tech_group, x='technology', y='Estimate', color='technology',
                title='Lead Value by Technology Interest', labels={'technology':'Technology','Estimate':'Value (€)'}
            )
            ...
            fig_industry = px.bar(
                industry_group, x='industry', y='Estimate', color='industry',
                title='Lead Value by Industry', labels={'industry':'Industry','Estimate':'Value (€)'}
            )
            ...
            fig_country = px.bar(
                country_group, x='country', y='Estimate', color='country',
                title='Lead Value by Country', labels={'country':'Country','Estimate':'Value (€)'}
            )
